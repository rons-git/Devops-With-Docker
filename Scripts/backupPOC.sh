#!/bin/bash
set +x && tput setaf 3 && echo "Backup POC Directory and Subdirectories" && tput setaf 21 
...
tput setaf 21 && echo -n "$(dirname "$basecopy")" contains "$(ls "$(dirname "$basecopy")" | wc -l)" directories \
